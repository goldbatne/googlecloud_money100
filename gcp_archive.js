  ...
  try {
    ...
    if (allGames.length > 0) {
      ...
      for (let idx = 0; idx < targetGames.length; idx++) {
        ...
        // ★ [NEW] 환각 방지, 특수문자 금지, 표 형식 강제가 추가된 V2.0 프롬프트
        const prompt = `
# Base Persona & Tone
...
- **[환각(Hallucination) 절대 금지]:** 레퍼런스 제안 시, 작동하지 않는 가짜 URL(링크)을 절대 지어내지 마십시오. 정확한 링크를 모른다면 "XX 게임 공식 포럼 개발자 노트 참조"와 같이 출처의 '명칭'이나 '검색 키워드'만 명시하십시오.
...
3. 반드시 유저가 게임 접속 후 **'실제 한국 서버 클라이언트 화면(UI)에서 직접 확인하고 클릭할 수 있는 버튼, 메뉴, 탭의 정확한 텍스트'**(예: 원신 - '기원', 승리의 여신: 니케 - '싱크로 디바이스')를 최우선으로 분석 대상으로 명시하십시오. 
...
* [특수문자 금지] Mermaid 노드 텍스트 괄호 \`[]\`, \`()\`, \`{}\` 내부에 콜론(\`:\`), 세미콜론(\`;\`), 따옴표(\`"\`, \`'\`), 쉼표(\`,\`)를 **절대 사용하지 마십시오**. (파서 에러의 주원인입니다. 예: \`A[UI:버튼]\` -> 에러, \`A[UI 버튼]\` -> 정상)

...
### 2.1 메인 시스템 구조도
확인된 서브시스템 간 연결 관계.
(★ Mermaid \`graph LR\` 강제 — 노드 5~8개. 단순하고 명확하게. 복잡한 레이블 금지)

### 2.2 서브시스템 구조도
가장 핵심적인 서브시스템 1개 내부 구조 상세화.
(★ Mermaid \`graph TD\` 강제 — 노드 4~6개. 단순하게)
...
### 3.1 메인 이용 플로우
유저의 핵심 이용 흐름 전체.
(★ Mermaid \`flowchart TD\` 강제 — 노드 6~10개. 핵심 경로만)

### 3.2 핵심 서브 플로우
가장 중요한 서브 플로우 1개 (획득·소비·강화 중 하나).
(★ Mermaid \`flowchart TD\` 강제 — 노드 4~8개)
...
### 4.4 상태 전이
확인된 시스템 내 주요 상태 변화.
(★ Mermaid \`stateDiagram-v2\` 강제 — 상태 3~5개. 단순하게)

---

# ★ 다이어그램 품질 기준 (Phase 1 최우선)
- **4개 필수**: 2.1(graph LR) · 2.2(graph TD) · 3.1(flowchart TD) · 4.4(stateDiagram-v2)
- **단순함이 정확함보다 중요**: 노드 수를 줄이더라도 문법 오류 없는 다이어그램 우선
- **노드 ID**: 영문+숫자만 (A1, B2, Node1). 한글·특수문자 절대 금지
- **레이블**: 10자 이내. 괄호·콜론·따옴표·쉼표 절대 금지
- **데이터 부족 시**: 확인된 2~3개 요소만으로 최소 구조 생성. 생략 금지
